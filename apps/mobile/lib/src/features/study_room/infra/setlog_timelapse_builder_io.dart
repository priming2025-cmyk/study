...
abstract final class SetlogTimelapseBuilderImpl {
  static Future<String?> buildAndShare({
    ...
    if (input.photos.isEmpty && input.clips.isEmpty) {
      // 요구사항: 안내 팝업 없이 “짧은 것이라도” 만들기
      // -> 프레임이 0이면 만들 수 있는 게 없어서 조용히 종료(에러 없음)
      return null;
    }
    ...
    final maxFrames = 360;
    final trimmed =
        frames.length > maxFrames ? frames.sublist(frames.length - maxFrames) : frames;
  ...
  static Future<Uint8List?> _renderFrameRgba({
    ...
    try {
      ...
      textPainter.text = const TextSpan(style: baseStyle, text: '공부 끝!');
      ...
      small.text = TextSpan(
        ...
        text: '시작 ${_fmt(startAt)}  ·  끝 ${_fmt(endAt)}  ·  다운로드 ${_fmt(downloadedAt)}',
      );
      ...
      if ((overlayLabel ?? '').trim().isNotEmpty) {
        ...
        tag.paint(canvas, const Offset(16, 16));
      }
